...
const jsxTagFor = (node: FigmaNode): string => {
	...
	const annotations = parseAnnotations(node.name);
	if (getImagePaint(node.fills)) return node.name.toLowerCase().includes("button") || annotations.on ? "imagebutton" : "imagelabel";
	if (node.name.toLowerCase().includes("button") || annotations.on) return "textbutton";
	return "frame";
};
